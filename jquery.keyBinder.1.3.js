...
This work is currently under development and may need additional work. 
...
$.fn.keyBinder= function(style, modifier, triggerOrDelay, functionToRun, functionToRunEnd) {
	...
	switch (style){
			...
			if (modifierArray && modifierPresent){ /* "ctrl+alt" && "a" */
				...
				thisEl.keydown(function (e) {
					if (e.keyCode == key[modifier[0]]){
						...
						functionToRun();
						thisEl.keyup(function (f) {
							if (f.keyCode == key[triggerOrDelay]){
								functionToRunEnd();
							}
						});
					}
		...
		case "sequence": /* "a b c d e f g h i j ctrl"*/
			...
			/*"sequence", "a b c d f e ctrl", function, function */
